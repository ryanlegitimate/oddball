...
TOTAL_TRIALS = 25
STIM_DURATION = 400
STANDARD_PULSE = 100  # Duration for non-target square pulse in ms
TARGET_PULSE = 400    # Duration for target square pulse in ms
ISI = 1000 
NUM_BLOCKS = 1
TARGET_PROB = 0.2
ISI_VARIES = False
...
while True:
        ...
        while trial_idx < TOTAL_TRIALS:
            ...
            while time.time() < stim_end_time:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                        ...
                        if stim_type == 'target' and not response_made:
                            all_logs.append({'block': block_num, 'trial': trial_idx + 1, 'stim_type': stim_type, 'reaction_time': rt, 'correct': 1, 'target': 1})
                            print(f"Block {block_num}, Trial {trial_idx + 1}, Stim: {stim_type}, RT: {rt} ms, Correct")
                            correct_chime.play()
                            response_made = True
                        else:
                            all_logs.append({'block': block_num, 'trial': trial_idx + 1, 'stim_type': stim_type, 'reaction_time': rt, 'correct': 0, 'target': 0 if stim_type == 'standard' else 1})
                ...
                if current_time >= pulse_end_time and square_active:
                    screen.fill(BLACK)  # Changed to BLACK background
                    if stim_type == 'standard' and STIM_TYPE in ["both", "visual"]:
                        pygame.draw.circle(screen, RED, (WIDTH // 2, HEIGHT // 2), 100)
                    elif stim_type == 'target' and STIM_TYPE in ["both", "visual"]:
                        pygame.draw.polygon(screen, GREEN, 
                                          [(WIDTH // 2, HEIGHT // 2 - 100), 
                                           (WIDTH // 2 - 100, HEIGHT // 2 + 100), 
                                           (WIDTH // 2 + 100, HEIGHT // 2 + 100)])
            ...
            while time.time() < end_isi_time:
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.VIDEORESIZE:
                        WIDTH, HEIGHT = event.w, event.h
                        screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE)
                    if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE and not response_made:
                        ...
                        if stim_type == 'target':
                            ...
                            all_logs.append({'block': block_num, 'trial': trial_idx + 1, 'stim_type': stim_type, 'reaction_time': rt, 'correct': 0, 'target': 0})
                            print(f"Block {block_num}, Trial {trial_idx + 1}, Stim: {stim_type}, RT: {rt} ms, Incorrect")
                            incorrect_chime.play()
            ...
            if not response_made:
                all_logs.append({
                    ...
                    'trial': trial_idx + 1,
                    'stim_type': stim_type,
                    'reaction_time': None,  # No reaction time if no response
                    'correct': 0 if stim_type == 'target' else 1,  # Correct if no response to standard, incorrect if no response to target
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['block', 'trial', 'stim_type', 'reaction_time', 'correct', 'target'])
        writer.writeheader()
